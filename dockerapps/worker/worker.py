...
url = "http://rng.{0}" .format(DOMAIN) + "/testing"
print (url)
...
redis = Redis("redis.service.{0}" .format(DOMAIN))


def get_random_bytes():
    r = requests.get("http://rng.service.{0}" .format(DOMAIN) + "/32")
    return r.content


def hash_bytes(data):
    r = requests.post("http://hasher.service.{0}" .format(DOMAIN) + "/",
                      data=data,
